...
class TestSyncJSONLsdcFields(unittest.TestCase):
    @parameterized.expand([
        ["with_sdc_fields", [{"id": 1}], {'id': 1, '_sdc_source_file': '/root_dir/data.jsonl', '_sdc_source_lineno': 1}],
        ["with_sdc_extra", [{"id": 1, "name": "abc"}], {'id': 1, 'name': 'abc', '_sdc_source_file': '/root_dir/data.jsonl', '_sdc_source_lineno': 1, '_sdc_extra': [{"name": "abc"}]}]
    ])
    def test_sync_JSONL(self, mocked_get_row_iterators, mocked_transform, mocked_sftp, name, test_data, expected_data):
        ...
        mocked_get_row_iterators.return_value = [('jsonl', test_data)]
    ...
    def test_sync_JSONL_empty_schema_with_records(self, mocked_get_row_iterators, mocked_transform, mocked_sftp):
        ...
        mocked_get_row_iterators.return_value = [('jsonl', [{"id": 1}])]
        conn = client.SFTPConnection("10.0.0.1", "username", port="22")
        table_spec = {
            "key_properties": [],
            "delimiter": None,
        ...
        args = mocked_transform.call_args.args
        records = args[0]
        self.assertEqual(records, {'id': 1, '_sdc_source_file': '/root_dir/data.jsonl', '_sdc_source_lineno': 1})
